...
export const coalPrimaryEntries: CoalNavItem[] = [
  ...
  { label: '设备管理', path: '/coal/equipment', desc: '设备状态、台账、维修与预测维护' },
...
export const coalTopicEntries: CoalNavItem[] = [
  ...
  { label: '生产消耗', path: '/coal/energy', desc: '水、电、介质、药剂、油脂与风量分析' },
...
/** 归入「生产管理」下拉的扩展入口（原扩展功能中与生产、消耗、排班强相关） */
export const coalProductionExtensionEntries: CoalNavItem[] = [
  ...
  { label: '水消耗', path: '/coal/water', desc: '补水汇总、历史分析与异常提示' },
  { label: '电力消耗', path: '/coal/power', desc: '配电状态、电耗台账与报警分析' },
...
export const coalExtensionEntries: CoalNavItem[] = [
  ...
  { label: '机电设备管理', path: '/coal/mechanical', desc: '设备档案、运行数据、二维码与闭环处理' },
  { label: '销售统计', path: '/coal/sales', desc: '按日期、产品、用户和运输方式统计销售质量' },
]

/** 首页扩展区展示用：生产专项 + 其余扩展（顺序与导航拆分一致） */
...
export const coalQuickNav: CoalNavItem[] = [
  { label: '首页', path: '/coal' },
  ...coalPrimaryEntries.map((item) => ({ label: item.label, path: item.path })),
  ...coalTopicEntries.map((item) => ({ label: item.label, path: item.path })),
  { label: '报表中心', path: '/coal/report' },
]
